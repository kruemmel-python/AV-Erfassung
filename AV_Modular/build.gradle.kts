...
val nativeConformance = tasks.register<Exec>("nativeConformance") {
    group = "verification"
...
val verifyPinnedActions = tasks.register("verifyPinnedActions") {
    group = "verification"
...
val avmInteroperability = tasks.register("avmInteroperability") {
    group = "verification"
...
val avmSpecificationArchive = tasks.register<Zip>("avmSpecificationArchive") {
    group = "distribution"
    description = "Creates the frozen AVM Specification 1.0.0-RC2 archive."
    from(layout.projectDirectory.dir("specification"))
    exclude("**/build/**", "**/.gradle/**")
    archiveFileName.set("AVM-Specification-1.0.0-RC2.zip")
...
tasks.register("avmReleaseCandidate") {
    group = "distribution"
    description = "Builds AVM 1.0.0-RC2 and creates signed manifest, report, SBOM and release envelope."
